def obtener_zona_horaria(coordenadas, fecha):
    ...
    basado en las coordenadas geográficas y los husos horarios
    """
    try:
        lat = coordenadas["lat"]
        lon = coordenadas["lon"]
        ...
        if "timezone" in tzdata:
            ...
            # Determinar huso horario basado en longitud (más preciso que país)
            huso_aproximado = round(lon / 15)  # 15 grados = 1 hora
            
            # Calcular diferencia entre huso teórico y real (para anomalías políticas)
            diferencia_huso = tz_offset - huso_aproximado
            ...
            return {
                ...
                "theoretical_offset": huso_aproximado,
                "offset_difference": diferencia_huso
            }
        
        # Si no hay datos, calcular huso horario aproximado basado en longitud
        huso_aproximado = round(lon / 15)  # 15 grados = 1 hora
        
        return {
            "name": f"UTC{'+' if huso_aproximado >= 0 else ''}{huso_aproximado}",
            "offset": huso_aproximado,
            "abbreviation_STD": f"UTC{'+' if huso_aproximado >= 0 else ''}{huso_aproximado}",
            "abbreviation_DST": f"UTC{'+' if huso_aproximado >= 0 else ''}{huso_aproximado}",
            "is_dst": False,
        ...
        # Si hay un error, intentar determinar el huso horario por la longitud
        try:
            lon = coordenadas["lon"]
            lat = coordenadas["lat"]
            huso_aproximado = round(lon / 15)  # 15 grados = 1 hora
            ...
            # Si todo falla, usar UTC como valor por defecto
            return {
                "name": "UTC",
                "offset": 0,
                ...
                "is_dst": False,
                "hemisphere": "norte",  # Default
                "theoretical_offset": 0,
                "offset_difference": 0,
                "estimated": True
            }
...
def convertir_a_utc(fecha, hora, timezone_info):
    ...
    try:
        ...
        if "theoretical_offset" in timezone_info:
            est_offset = timezone_info["theoretical_offset"]
...
def preload_resources():
    ...
    # Cargar zonas horarias - ahora es opcional ya que usamos cálculo por coordenadas
    try:
        time_zone_df = pd.read_csv("time_zone.csv", 
                              names=["timezone", "country_code", "abbreviation", "timestamp", "utc_offset", "dst"],
                              header=None)
        print("Archivo de zonas horarias cargado correctamente")
    except Exception as e:
        print(f"No se pudo cargar el archivo de zonas horarias: {e}")
        print("Se utilizará cálculo geográfico de zona horaria")
        time_zone_df = None
...
def calculate_positions(date_str, time_str, lat=None, lon=None):
    try:
        ...
        # Calcular zona horaria por longitud si no se utiliza otra
        if lon is not None:
            est_offset = round(lon / 15)  # 15 grados = 1 hora
            loc_tz = timezone(timedelta(hours=est_offset))
        else:
            # Zona por defecto (Europa/Madrid)
            loc_tz = timezone(timedelta(hours=1))
            
        local_dt = local_dt.replace(tzinfo=loc_tz)
    ...
    try:
        ...
        try:
            ...
            response = {
                ...
                "aspects": [],
                "dignity_table": {"tabla": [], "total_general": 0},
                "houses_analysis": {
                    "houses": [],
                    "birth_type": "húmedo"
                },
            ...
            # Generar huso horario aproximado basado en longitud
            lon = city_data["lon"]
            lat = city_data["lat"]
            estimated_offset = round(lon / 15)  # 15 grados = 1 hora
            
            timezone_info = {
                "name": f"UTC{'+' if estimated_offset >= 0 else ''}{estimated_offset}",
                "offset": estimated_offset,
                "abbreviation_STD": f"UTC{'+' if estimated_offset >= 0 else ''}{estimated_offset}",
                "is_dst": False,
                "hemisphere": "norte" if lat >= 0 else "sur",
                "estimated": True,
                "theoretical_offset": estimated_offset
            }
            ...
            return jsonify({
                ...
                "city": city_data["nombre"],
                "timezone": timezone_info,
                "timezone_note": "Zona horaria aproximada basada en coordenadas geográficas",
                "aspects": [],
                "dignity_table": {"tabla": [], "total_general": 0},
                "houses_analysis": {
                    "houses": [],
...
# Función para determinar huso horario basado únicamente en coordenadas
def determinar_huso_horario_por_coordenadas(lat, lon):
    """
    Determina el huso horario teórico basado puramente en la longitud geográfica
    """
    # Calcular huso horario: cada 15 grados es una hora
    huso = round(lon / 15)
    
    # Determinar si es hemisferio norte o sur
    hemisferio = "norte" if lat >= 0 else "sur"
    
    # Determinar si está en verano o invierno (aproximación muy simple)
    # Una implementación real requeriría conocer la fecha y aplicar reglas específicas
    mes_actual = datetime.now().month
    es_verano_norte = 4 <= mes_actual <= 9
    es_verano_sur = not es_verano_norte
    
    es_verano = (hemisferio == "norte" and es_verano_norte) or (hemisferio == "sur" and es_verano_sur)
    
    # Crear objeto de información de zona horaria
    return {
        "name": f"UTC{'+' if huso >= 0 else ''}{huso}",
        "offset": huso,
        "abbreviation_STD": f"GMT{'+' if huso >= 0 else ''}{huso}",
        "abbreviation_DST": f"GMT{'+' if huso+1 >= 0 else ''}{huso+1}",
        "is_dst": es_verano,
        "hemisphere": hemisferio,
        "theoretical_offset": huso,
        "estimated": True
    }
