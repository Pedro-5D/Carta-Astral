...
def get_cities():
    ...
    url = f"https://api.geoapify.com/v1/geocode/autocomplete?text={ciudad}&apiKey={api_key}&limit=20&type=city"
    ...
    try:
        ...
        # Si no encontramos con Geoapify, intentar con OpenStreetMap Nominatim
        if len(ciudades) < 3:
            print("Pocos resultados de Geoapify, intentando con Nominatim")
            try:
                # Configurar cabeceras para Nominatim (requisito de su API)
                headers = {
                    'User-Agent': 'CartaAstral/1.0',
                    'Accept-Language': 'es,en'
                }
                
                # Hacer búsqueda en Nominatim
                nom_url = f"https://nominatim.openstreetmap.org/search?q={ciudad}&format=json&addressdetails=1&limit=15"
                nom_response = requests.get(nom_url, headers=headers, timeout=10)
                nom_data = nom_response.json()
                
                for place in nom_data:
                    # Construir un nombre formateado
                    if "display_name" in place:
                        city_name = place["display_name"]
                        if city_name not in ciudades:
                            ciudades.append(city_name)
            except Exception as e:
                print(f"Error en búsqueda Nominatim: {str(e)}")
        
        # Si sigue sin haber suficientes resultados, intentar con búsqueda global en Mapbox
        if len(ciudades) < 2:
            print("Pocos resultados, intentando con otra fuente")
            try:
                # Lista de países donde podría haber ciudades con este nombre
                paises = ["España", "México", "República Dominicana", "Venezuela", "Estados Unidos", 
                          "Argentina", "Colombia", "Ecuador", "Chile", "Perú", "Bolivia", 
                          "Uruguay", "Paraguay", "Cuba", "Honduras", "El Salvador", "Costa Rica", 
                          "Panamá", "Guatemala", "Brasil", "Francia", "Italia", "Alemania", 
                          "Reino Unido", "Portugal"]
                
                # Generar algunas opciones plausibles
                for pais in paises:
                    ciudades.append(f"{ciudad}, {pais}")
                    
                # Limitar a 10 opciones para no sobrecargar al usuario
                ciudades = ciudades[:10]
            except Exception as e:
                print(f"Error generando opciones: {str(e)}")
        ...
        # En caso de error, intentar generar opciones de países
        try:
            paises = ["España", "México", "República Dominicana", "Venezuela", "Estados Unidos", 
                      "Argentina", "Colombia", "Ecuador", "Chile", "Perú", "Bolivia", 
                      "Uruguay", "Paraguay", "Cuba", "Honduras", "El Salvador", "Costa Rica", 
            ...
            ciudades_opciones = [f"{ciudad}, {pais}" for pais in paises[:10]]
            return jsonify({"ciudades": ciudades_opciones})
        except:
            # Si todo falla, al menos dar 3 opciones
