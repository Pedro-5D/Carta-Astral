...
def determinar_horario_verano(fecha, hemisferio, coordenadas):
    ...
    Basado en el equinoccio de primavera para el hemisferio norte
    y el equinoccio de otoño para el hemisferio sur
    """
    año = fecha.year
    mes = fecha.month
    dia = fecha.day
    ...
    if "spain" in pais or "españa" in pais:
        ...
        if año < 1974:
            ...
            # Para años posteriores, usar las fechas aproximadas del equinoccio
            if hemisferio == "norte":
                # Equinoccio de primavera ~21 de marzo al equinoccio de otoño ~22 de septiembre
                if (mes > 3 and mes < 9) or (mes == 3 and dia >= 21) or (mes == 9 and dia <= 22):
                    return True
            else:
                # Equinoccio de otoño ~20 de marzo al equinoccio de primavera ~23 de septiembre
                if (mes < 3) or (mes > 9) or (mes == 3 and dia <= 20) or (mes == 9 and dia >= 23):
                    return True
            return False
    
    # Reglas para otros países basadas en equinoccios
    else:
        if hemisferio == "norte":
            # Hemisferio Norte: DST desde equinoccio de primavera (~21 marzo) hasta equinoccio de otoño (~22 septiembre)
            if (mes > 3 and mes < 9) or (mes == 3 and dia >= 21) or (mes == 9 and dia <= 22):
                return True
        else:
            # Hemisferio Sur: DST desde equinoccio de primavera austral (~22 septiembre) hasta equinoccio de otoño austral (~21 marzo)
            if (mes < 3) or (mes > 9) or (mes == 3 and dia <= 20) or (mes == 9 and dia >= 23):
                return True
        return False
