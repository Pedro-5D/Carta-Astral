...
def preload_resources():
    global eph, ts, interpreter, common_cities
    ...
    try:
        ...
        try:
            eph = load.one_file('https://naif.jpl.nasa.gov/pub/naif/generic_kernels/spk/planets/de435.bsp')
        except:
            print("Usando fuente alternativa de efemérides")
            # En caso extremo, usar skyfield.iokit.SpiceKernel
            from skyfield.iokit import SpiceKernel
            eph = SpiceKernel('de421.bsp')
    
    ts = load.timescale()
    
    try:
        # Cargar intérprete si existe el archivo XML
