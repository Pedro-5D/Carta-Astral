def obtener_zona_horaria(coordenadas, fecha):
    ...
    try:
        ...
        # Si no hay datos, usar UTC como valor por defecto
        return {
            "name": "UTC",
            "offset": 0,
        ...
        raise Exception(f"No se pudo determinar la zona horaria para las coordenadas dadas: {str(e)}")

def convertir_a_utc(fecha, hora, timezone_info):
    """
    Convierte fecha y hora local a UTC considerando zona horaria y DST
    La API de Geoapify ya considera si es horario de verano o no en cada hemisferio
    """
    try:
        ...
        # Si estamos en DST, ajustar según lo que indica la API
        if timezone_info["is_dst"]:
            # Geoapify ya proporciona el offset correcto incluyendo DST
            # No es necesario ajustar manualmente
            pass
